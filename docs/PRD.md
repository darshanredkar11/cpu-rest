...
- Every request/response DTO must be a **public** type with a `public static final json.JsonCodec<T> CODEC` field (`com.cpurest.util.Wire` looks this up once, via `Class.getField`, and caches it — see `docs/WHITEPAPER.md` §6). There is no reflective auto-mapping of arbitrary POJOs; the codec is hand-written, matching `json-serializer`'s own no-reflection design.
...
- **Single in-flight request per bus.** Each channel is a single-slot SPSC mailbox, not a multi-slot ring — concurrent calls through the same client (or multiple clients on the same bus) serialize through an internal lock. Sufficient for the synchronous request/response pattern this version targets; see Roadmap for pipelining.
...
- **Every DTO needs a hand-written `JsonCodec`.** There's no reflective auto-mapping (see API Surface above) — a DTO with many fields means writing all of them out in a `write`/`read` pair. This is a deliberate trade against `json-serializer`'s own no-reflection design (see `docs/WHITEPAPER.md` §6), not an oversight, but it is more upfront work per type than Jackson-style auto-mapping.
